...
For Node-based repositories, packaging entry points can be declared explicitly in `package.json` as `package:windows` and `package:android`. FolderBridge discovers those fixed script names at call time and executes them only as `npm run package:windows` / `npm run package:android`; it never copies the script body into a shell command. Existing bounded Windows PowerShell/PyInstaller and Android Gradle/Flutter discovery remains available as a compatibility fallback.
