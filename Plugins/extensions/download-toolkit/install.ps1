...
if (Test-Path -LiteralPath $target) {
    ...
    if ([IO.Path]::GetFullPath($targetItem.FullName) -eq [IO.Path]::GetFullPath($sourceRoot)) {
        Write-Host "Download Toolkit is already running from the target hot-load directory: $target"
        exit 0
    }
}
...
try {
    ...
    Write-Host "Installed external Download Toolkit Extension: $target"
