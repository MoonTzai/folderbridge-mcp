...
$extensionId = "windows-capture-toolkit"
...
if (Test-Path -LiteralPath $target) {
    ...
    if ([IO.Path]::GetFullPath($targetItem.FullName) -eq [IO.Path]::GetFullPath($sourceRoot)) {
        Write-Host "Windows Capture Toolkit is already running from the target hot-load directory: $target"
        exit 0
    }
}
...
try {
    ...
    Write-Host "Installed external Windows Capture Toolkit Extension: $target"
    Write-Host "Open FolderBridge Extensions & Skills, click Rescan, then approve the exact new hash and enable it."
}
catch {
