...
| `comfyui` | 1.5.0 | Hot-load bridge to local ComfyUI with host-owned workflow Jobs, cross-project WebSocket-free health snapshots, targeted prompt cancellation, bounded real-job/model/node inspection, explicit MiniMax Director progress diagnostics, explicit memory release, transient history retry, and optional scoped workspace output. |
