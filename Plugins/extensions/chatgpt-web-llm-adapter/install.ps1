...
$extensionId = "chatgpt-web-llm-adapter"
...
if (Test-Path -LiteralPath $target) {
    ...
    if ([IO.Path]::GetFullPath($targetItem.FullName) -eq [IO.Path]::GetFullPath($sourceRoot)) {
        Write-Host "ChatGPT Web LLM Test Adapter is already running from the hot-load directory: $target"
        exit 0
    }
}
...
try {
    ...
    foreach ($name in @("folderbridge-extension.json", "plugin.py", "browser_runtime.py", "standalone.py", "launch-standalone.ps1", "README.md", "install.ps1")) {
    ...
    Write-Host "Installed external ChatGPT Web LLM Test Adapter: $target"
    Write-Host "Installed Standalone-capable package. The repo-root Standalone launcher does not require FolderBridge approval."
    Write-Host "For optional Extension mode: open FolderBridge Extensions & Skills, click Rescan, review the permissions/hash, then approve and enable it."
}
catch {
