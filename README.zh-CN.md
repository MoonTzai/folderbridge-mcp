...
1. 在文件夹列表中逐个添加明确的工作区（最多 16 个），首次使用保持“只读（推荐）”；全局权限作用于列表内全部目录。
...
ComfyUI 现在是标准的**外源热加载 Extension**，公开源位于 `Plugins/extensions/comfyui`，并且明确不再打进 `FolderBridge.exe`。状态检查固定访问 `127.0.0.1:8188`；执行 workflow 需要在 Extensions 侧栏对精确目录 hash 批准。Windows Launcher 仍可以独立托管本地 ComfyUI 进程：只需选择一次受支持的 Portable 根目录（`python_embeded\\python.exe` + `ComfyUI\\main.py`），或源码安装根目录（`main.py` + `.venv\\Scripts\\python.exe` / `venv\\Scripts\\python.exe`），之后可用明确的 Python/main.py 参数、`shell=False`、固定 `--fast-disk` 和固定 `127.0.0.1:8188` 自动启动。如果 8188 在 FolderBridge 启动前已经在线，则标记为外部服务并直接复用，不会再启动第二份。Launcher managed-service controller 与外源 Extension runtime 是刻意分离的两层。
