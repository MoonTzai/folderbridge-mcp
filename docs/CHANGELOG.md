...
- Synchronized the public Extension source and Release surface with accepted local runtimes: **Storyboard ChatGPT Web 0.2.0** and **Windows Capture Toolkit 0.1.3** are now public optional plugins alongside ChatGPT Web LLM Adapter 0.3.2. The public catalog is 11 external Extensions; curated Releases contain one Windows executable plus 11 Plugin ZIPs. Explicit minimal file allowlists are member-verified for LLM Adapter, Storyboard ChatGPT Web, and Windows Capture, while the private Debate Judge adapter remains excluded.
- Fixed a live-observed LLM Bridge credential-generation race: after a managed Standalone restart the Launcher sidebar could already read the new temporary API key from `connection.json` while an existing or prematurely reopened `status.html` browser document still displayed the previous generation's key.
- A managed ChatGPT Web LLM Bridge start now removes the previous `status.html` before spawning the new generation and fails closed if that stale page cannot be cleared. Opening the status page additionally verifies that its embedded one-run key matches the current live connection record before navigation.
- Status-page navigation now uses a cache-busted `file://` URL in a new browser tab so browser reuse cannot preserve a stale in-memory document across Standalone generations. That host-only correction itself did not require plugin reinstall or renewed exact-hash approval.
- External **ChatGPT Web LLM Adapter 0.3.2** adds history-safe fresh-chat throttling: 20-second minimum starts, a rolling 8-per-5-minute cap, known conversation-history rate-limit detection with 2/4/8/10-minute cooldown, and login-page reuse. Temporary Chat remains explicitly not enforced until its web control is reliably verifiable.
- External **ChatGPT Web LLM Adapter 0.3.1** hardens strict JSON extraction after a real Cognitive Landscape low-sample regression exposed an intermittent false 502: assistant-message UI controls are removed before text extraction, and harmless `json` / `Copy code` display labels may be joined by layout whitespace without failing the one-object gate. Semantic prose, arrays, multiple objects, and malformed JSON remain rejected.
- 0.8.38 source regression after public-plugin synchronization: **767 tests GREEN, 3 skipped** on the real local Windows desktop. The two Windows Capture live desktop-enumeration cases remain enabled locally but are skipped on headless GitHub Actions Windows runners; manifest, path/hash, PNG, capture-mode and safety-contract coverage still runs in CI. Windows package build and bundle verification are GREEN; packaged `FolderBridge.exe` smoke reports `folderbridge-mcp 0.8.38`, with 11 external Plugin ZIPs plus internal SHA sidecars generated successfully. Final live restart/status-page acceptance is tracked separately because it requires running the newly built Launcher generation.

## 0.8.37 — 2026-09-18

- Added Launcher-owned one-click management for the external **ChatGPT Web LLM Adapter 0.3.0** inside **Extensions & Skills**: explicit `OFFLINE / STARTING / WAITING_LOGIN / READY / ERROR`, Start/Restart/Stop, separate status/login-page controls, a real fixed JSON completion validation that reports `LIVE_PROBE_PASS`, Base URL / model / per-run API-key copy fields, active/max request visibility, and a bounded 1–4 concurrency slider with default 2. The validation result itself never carries the temporary bearer.
- Preserved strict process ownership: the Launcher stops only the Standalone process tree it created and still owns by process handle. Unknown/external occupancy on 8769/8770 fails closed and is never converted into PID-by-port termination. Restart first stops the owned process/browser, waits for release, then starts the currently exact-hash-approved installed Extension and receives a fresh one-run token.
- Split version truth across source/manifest, installed exact hash, live Adapter `/health`, and generated `status.html`; an old or mismatched live process is `ERROR`, never READY. Extension `status/connection` no longer returns the one-run bearer to MCP callers; the credential remains on the local human control plane.
- Promoted `chatgpt-web-llm-adapter` to the ninth public external Extension. Windows packaging uses an explicit seven-file release allowlist and verifies the resulting ZIP member-for-member, excluding local HANDOFF/CLOSURE material. The curated GitHub Release surface is now one Windows executable plus nine optional Plugin ZIPs.
- Added a small generic **Install/update plugin ZIP…** seam to Extensions & Skills so external plugin updates no longer require CMD/file-copy workflows. ZIP input is staged and bounded; traversal, links/reparse-style entries, encryption, case-colliding members, oversized expansion, wrapper-folder manifests, and attempts to shadow release-bundled IDs are rejected. A managed service is stopped only through its current Launcher-owned controller before replacement, and changed hashes deliberately invalidate rather than inherit approval.
- Final source regression before live cutover: **756 tests GREEN, 3 skipped**. Windows 0.8.37 packaging and live installed/runtime acceptance remain separate gates and are recorded only after the corresponding artifacts/runtime are verified.
...
- Added the external **File Ops Toolkit 0.1.0** source and installer for bounded workspace file copy/move operations without reintroducing arbitrary shell access.
- Added the external **video-storyboard-production 1.0.0 Skill Pack**, including continuity inventory, generated-video review, MiniMax H3 prompt/workflow, narration orchestration, narration-to-storyboard, and shot-specification methods plus upstream license/notice material.
- Finished Runtime API Key edge-whitespace handling at the launcher UI boundary as well as the backend boundary: pasted leading/trailing spaces, tabs, CR/LF and other edge whitespace are stripped immediately from the in-memory entry state, FocusOut/start/apply/diagnose remain defensive normalization points, and inherited `CONTROL_PLANE_API_KEY` values receive the same bounded normalization. Credentials remain memory-only and are never written to launcher settings or logs.
- Raised the shared MCP request framing ceiling from the legacy 1 MiB limit to **32 MiB** through one transport-limit contract used by production stdio and the Phase-0 private HTTP ingress. This is a transport framing bound, not a file-size regression: SHA-locked exact text editing remains **128 MiB**, streaming literal search remains **512 MiB**, and transactional whole-file writes remain **512 MiB**.
- Raised transactional `write_file append` chunks from 128 KiB to **4 MiB**. The chunk size remains conservatively below the 32 MiB framing ceiling even under pathological JSON escaping, reducing large whole-file write round trips without weakening atomic begin/append/commit semantics.
- Fixed the shared-stdio global-drop chain triggered by an oversized request. FolderBridge now emits a bounded request-correlated JSON-RPC error **before** draining the rejected line, preserves integer or bounded string request IDs when recoverable from the prefix, then reframes the stream. A malformed/oversized logical request no longer needs to turn into an `id:null` terminal response that a shared-stdio tunnel can interpret as a protocol failure and close for every client.
- Expanded Flight Recorder oversize diagnostics without recording payload bodies: `request_bytes` is explicitly marked as a lower bound when the newline has not yet been observed, bounded structural identity is captured, and post-drain telemetry records observed bytes, newline termination, and EOF-during-discard.
- Added regression coverage for direct stdio requests above the former 1 MiB boundary, 32 MiB oversize request isolation with string IDs, the aligned 32 MiB private HTTP ingress, and official tunnel-client v0.0.14 dev-proxy → shared stdio → real FolderBridge traversal at **2/4/8/16 MiB**, followed by an over-ceiling request and a successful same-runtime recovery call. Full source suite: **676 tests GREEN, 4 skipped**.
...
- Advanced bundled Git Publisher to 1.4.0 and removed the legacy FolderBridge-specific `release` action plus its version/asset helpers. Git Publisher is now project-agnostic: `status`, browser `connect`, explicit `commit`, current-branch `push`, and generic `release-assets` only. FolderBridge's own version/build/tag/fixed-asset release policy remains solely in `.github/workflows/release-windows.yml`, which independently tests, builds, verifies, tags, and creates/repairs the GitHub Release.
- Audited repository hygiene after the ComfyUI externalization: current public docs now consistently describe the 16-workspace limit, external ComfyUI boundary, and current Git Publisher tracked-deletion contract; public/bundled/private source roles are explicit; local-only Judge/discipline assets are protected from accidental publication; and repository-hygiene regression tests lock these cross-file contracts against future drift.
