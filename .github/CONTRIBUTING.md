...
Windows release builds use the exact PyInstaller version in `packaging/requirements-build.txt` and `scripts/build_windows.ps1`. Do not commit generated `.build`, `.build-venv`, `release`, or executable files; release binaries belong in GitHub release assets.
